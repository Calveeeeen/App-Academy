...
async function workout(totalTime) {
  try{
    let timeMinusStretch = await stretch(totalTime);
    let timeMinusRun = await runOnTreadmill(timeMinusStretch);
    let timeMinusLift = await liftWeights(timeMinusRun);
    console.log(`done working out with ${timeMinusLift/1000} seconds left`);
  }
  catch(err){
